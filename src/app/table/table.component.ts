...
export class TableComponent implements OnInit {
  ...
  getDeepData(key) {
      ...
      let item = this.deepLevelData.find((i) => i.key === key);
      if (!item) {
          item = {
              key: key,
              dataSourceInstance: new DataSource({
          ...
          this.deepLevelData.push(item)
      }
      return item.deepLevelData;
  }
  ...
  onRowUpdated({data}) {
    ...
    const filteredRecord = this.tasksDataSourceStorage.filter((data) => data.key === changedId)
    const filteredItems = filteredRecord.length ? filteredRecord[0].dataSourceInstance.items() : []
